...
SELECT ab_trace_tag,
  ...
  FROM log.ads_sead_bizer_recallad_dp
 where dt = '20200115'
   and slot_id = 10002
 limit 100
```
...
```
# hive
select slot_id, request_id, source, content, id from mart_ads.creative_zgs_dp_runlog
...
create table tmp.expanded_recall_0115 as SELECT request_id,
       shop_id,
       launch_id,
       json_extract(CAST(extensions AS VARCHAR),'$.zgs.defineTalkSwitch') as defineTalk,
       json_extract(CAST(extensions AS VARCHAR),'$.zgs.smartTalkSwitch') as samrtTalk,
       json_extract(CAST(extensions AS VARCHAR),'$.zgs.zgsMaterial.source')as source,
       json_extract(CAST(extensions AS VARCHAR),'$.zgs.zgsMaterial.id') as id,
       json_extract(CAST(extensions AS VARCHAR),'$.zgs.zgsMaterial.content')as content,
       url
  from (
        SELECT request_id,
...
SELECT request_id,
  ...
  from (
        SELECT request_id,
               ...
               dt,
               hour
          FROM log.ads_sead_bizer_recallad_dp lateral view explode(split(ads, '\\},\\{')) tb as tmp_col
...
```sql
-- Hive
select unix_timestamp('20190909','yyyyMMdd')  from dw.dim_dp_shop
...
select mt_poi_id, 
    ...
    tag
from tmp.creative_poi_xgtag_v1 
...
#### case语句
```
sum(CASE WHEN (validclickcnt > 0) THEN 1 ELSE 0 END) as click,
```
...
SELECT shopid,
  ...
  FROM mart_ads.topic_sh_ctrcvr_labeled_daily
